 ...
 * @deprecated No se debe usar mas, se debe reemplazar por FileDiscoverer
 */
public class FileVisitor implements IResourceVisitor {

	private Map<Locale, IFile> locales;
	private IWildcardPath wp;
	private IWildcardPath comparator;

	public FileVisitor(Map<Locale, IFile> locales, IWildcardPath wp) {
	...
	public boolean visit(IResource resource) throws CoreException {
		...
		if (resource.getType() == IFile.FOLDER) {
			...
			boolean isFilename = wp.getFileName().equals(
					comparator.getFileName());
			boolean isFileExtension = wp.getFileExtension().equals(
					comparator.getFileExtension());
